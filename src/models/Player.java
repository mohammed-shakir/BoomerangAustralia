...
public class Player {
    ...
    public int roundScore(int roundNr) {
        ...
        // Fix total score and round score... Also, sum up the round score according to
        // the scoring sheet
        ...
        sendMessage("This round you scored " + thisRoundSites + " new sites points and " + regionRoundScore
        ...
        if (roundNr == 3) {// last round
            String t = "Throw and Catch score", to = "Tourist sites score", c = "Collections score",
                    a = "Animals score";
            int totalAct = 0;
            ...
            int totalT = rScore.get(0).get(t).intValue() + rScore.get(1).get(t).intValue()
                    + rScore.get(2).get(t).intValue() + rScore.get(3).get(t).intValue();
            int totalTo = rScore.get(0).get(to).intValue() + rScore.get(1).get(to).intValue()
                    + rScore.get(2).get(to).intValue() + rScore.get(3).get(to).intValue();
            int totalC = rScore.get(0).get(c).intValue() + rScore.get(1).get(c).intValue()
                    + rScore.get(2).get(c).intValue() + rScore.get(3).get(c).intValue();
            int totalA = rScore.get(0).get(a).intValue() + rScore.get(1).get(a).intValue()
                    + rScore.get(2).get(a).intValue() + rScore.get(3).get(a).intValue();
            // Requirement 12
            String finalScore = "                       Round 1\tRound 2\tRound 3\tRound 4\tTotal\n";
            finalScore += "Throw and Catch score:   " + rScore.get(0).get(t) + "\t  " + rScore.get(1).get(t)
                    + "\t  " + rScore.get(2).get(t) + "\t  " + rScore.get(3).get(t) + "\t  " + totalT + "\n";
            finalScore += "  Tourist sites score:   " + rScore.get(0).get(to) + "\t  " + rScore.get(1).get(to)
                    + "\t  " + rScore.get(2).get(to) + "\t  " + rScore.get(3).get(to) + "\t  " + totalTo + "\n";
            ;
            finalScore += "    Collections score:   " + rScore.get(0).get(c) + "\t  " + rScore.get(1).get(c)
                    + "\t  " + rScore.get(2).get(c) + "\t  " + rScore.get(3).get(c) + "\t  " + totalC + "\n";
            finalScore += "        Animals score:   " + rScore.get(0).get(a) + "\t  " + rScore.get(1).get(a)
                    + "\t  " + rScore.get(2).get(a) + "\t  " + rScore.get(3).get(a) + "\t  " + totalA + "\n";
            ...
            finalScore += "          Total score:   " + (totalT + totalC + totalA + totalAct + (region.size() * 3))
                    + " points\n";
            sendMessage("\n*********************************************\n" + finalScore);
            return (totalT + totalC + totalA + totalAct + (region.size() * 3));
        }
        int totalRoundScore = regionRoundScore + throwCatchScore + thisRoundSites + countRoundCollections
                + countRoundAnimals + countRoundActivities;
        regionRoundScore = 0; // it has been added to the region list, and will be summed up at the end of the
                              // 4 rounds.
